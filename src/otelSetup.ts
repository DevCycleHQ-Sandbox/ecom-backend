...
let traceSpanProcessor: SpanProcessor | undefined // Keep reference for potential manual shutdown or monitoring
...
export function initializeOpenTelemetry() {
  ...
  for (const name of [
    ...
    try {
      ...
      // console.debug(`Failed to read Dynatrace metadata from ${name}:`, error.message) // Optional: for debugging
    }
  }
  ...
  try {
    ...
    // Mark as configured only after successful initialization
    configured = true
  } catch (error: unknown) {
    console.error(
